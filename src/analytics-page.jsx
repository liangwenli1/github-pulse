...
export function AnalyticsPage({l,names,updatePath}){
  ...
  {error?<div className="analytics-state" role="alert">{t.error} <button type="button" onClick={()=>setRevision(v=>v+1)}>{t.retry}</button></div>:loading?<div className="analytics-loading" role="status">{t.loading}</div>:items.length===0?<div className="analytics-state">{t.empty}</div>:<><div className="analytics-meta"><span>{ranking?.source==='demo'?t.demo:t.source}</span><span>{t.topNote.replace('{count}',num(items.length,l))}</span>{ranking?.updatedAt&&<span>{t.updated} {new Date(ranking.updatedAt).toLocaleDateString(l==='zh'?'zh-CN':'en-US')}</span>}</div><div className="analytics-scene-body analytics-single">

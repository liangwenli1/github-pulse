...
const tr=process.env.SMTP_HOST?nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT||587),secure:Number(process.env.SMTP_PORT||587)===465,auth:process.env.SMTP_USER?{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS}:undefined}):null;
...
export async function sendMail(to,subject,text,html,headers,idempotencyKey) {
  if ((process.env.DATA_MODE||'demo')==='demo') {
    db.prepare('INSERT INTO outbox (to_email,subject,text,html,created_at) VALUES (?,?,?,?,?)').run(to,subject,text,html,new Date().toISOString());
  ...
  if (process.env.RESEND_API_KEY&&process.env.RESEND_FROM) {
    const response=await fetch('https://api.resend.com/emails',{
      method:'POST',
      headers:{'Authorization':`Bearer ${process.env.RESEND_API_KEY}`,'Content-Type':'application/json','User-Agent':'GitHubPulse/0.1.0',...(idempotencyKey?{'Idempotency-Key':idempotencyKey}:{})},
  ...
  if(tr&&process.env.SMTP_FROM){await tr.sendMail({from:process.env.SMTP_FROM,to,subject,text,html,headers});return;}
...
export async function sendVerification(sub,token) {
  ...
  const subject=zh?'验证你的 GitHub Pulse 订阅':'Verify your GitHub Pulse subscription';
  const text=(zh?'请点击以下链接验证邮箱，验证后才会开始发送每日摘要：':'Open this link to verify your email before daily digests begin:')+`\n${link}`;
  const html=`<div style="font:16px/1.6 system-ui,sans-serif;max-width:560px;margin:auto;padding:24px;color:#172327"><h1 style="font-size:24px">GitHub Pulse</h1><p>${zh?'请验证邮箱以启用每日摘要。':'Verify your email to enable your daily digest.'}</p><p><a href="${escape(link)}" style="display:inline-block;background:#087e61;color:#fff;padding:12px 18px;border-radius:8px">${zh?'验证邮箱':'Verify email'}</a></p><p>${escape(link)}</p></div>`;
  await sendMail(sub.email,subject,text,html);
}

export function buildDigest(sub,manageToken) {
  const zh=sub.locale==='zh';
  const names=JSON.parse(sub.boards);
  const sections=[];
  for(const board of names) {
    if(!boards[board]) continue;
    const ranking=getRankings({board,period:'day',language:sub.language||'',topic:sub.topic||'',limit:5});
  ...
  const subject=zh?'GitHub Pulse 每日摘要':'GitHub Pulse daily digest';
  const intro=zh?'你关注的开源项目榜单':'Your open-source discovery digest';
  const lines=[intro,'',...sections.flatMap(s=>[`${s.name} · ${localTime(s.updatedAt)} (${sub.timezone})`,...s.items.map(r=>`#${r.rank} ${r.full_name} · ${r.gain===null?(zh?'数据不足':'Insufficient data'):`+${num.format(r.gain)} ★`} · ${r.url}`),boardLink(s.board),'']),zh?'管理订阅：':'Manage subscription:',manage,zh?'一键退订：':'Unsubscribe:',unsubscribe];
  const html=`<div style="background:#f5f7f7;padding:16px"><main style="max-width:600px;margin:auto;background:#fff;padding:24px;font:15px/1.6 system-ui,sans-serif;color:#172327"><h1 style="margin:0">GitHub Pulse</h1><p>${intro}</p>${sections.map(s=>`<section style="border-top:1px solid #dce4e5;padding:18px 0"><h2 style="font-size:19px">${escape(s.name)}</h2><p style="font-size:12px;color:#526267">${zh?'数据采样':'Sampled'}: ${escape(localTime(s.updatedAt))} (${escape(sub.timezone)})${(process.env.DATA_MODE||'demo')==='demo'?' · DEMO DATA':''}</p>${s.items.map(r=>`<p style="margin:10px 0"><strong>#${r.rank}</strong> <a href="${escape(r.url)}">${escape(r.full_name)}</a><br><span style="color:#526267">${r.gain===null?(zh?'数据不足':'Insufficient data'):`+${num.format(r.gain)} ★`}</span></p>`).join('')}<a href="${escape(boardLink(s.board))}">${zh?'查看完整榜单':'View full board'}</a></section>`).join('')}<footer style="border-top:1px solid #dce4e5;padding-top:16px;font-size:13px"><a href="${escape(manage)}">${zh?'管理订阅':'Manage'}</a> · <a href="${escape(unsubscribe)}">${zh?'一键退订':'Unsubscribe'}</a></footer></main></div>`;

...
process.env.DB_PATH=path.resolve('../../work/api-test-'+process.pid+'.sqlite');
...
test('verification gates delivery and management controls the subscription',async()=>{
  ...
  try {
    ...
    const created=await request('/api/subscriptions','POST',{email:'hello@example.invalid',locale:'zh',boards:['hot','ai'],sendHour:9,timezone:'Asia/Shanghai'});
    assert.equal(created.status,201);
    assert.equal(db.prepare("SELECT status FROM subscriptions WHERE email='hello@example.invalid'").get().status,'pending');
    assert.equal((await digest({force:true})).sent,0);
    const text=db.prepare('SELECT text FROM outbox ORDER BY id DESC LIMIT 1').get().text;
    ...
    const mail=db.prepare('SELECT text FROM outbox ORDER BY id DESC LIMIT 1').get().text;

...
The centered header links Home, Rankings, Charts, Subscribe, and About. The homepage at `/en/home` (or `/zh/home`) has an introduction and chart overview. The "View charts" action sits below the four overview charts, followed by the subscription area at the end of the homepage. The Subscribe navigation link scrolls to that area; it can be reached directly with `#subscribe`. The interactive rankings live at `/en/ranking`; the full chart story lives at `/en/charts`. Scrolling snaps to screens on the overview and chart pages; headings and content fade and rise into place, while a small page indicator permits direct navigation. The overview holds cumulative new Stars, daily new Stars or net changes, top-five comparison, and language composition in the current top 50. Charts follow the selected board and filters. A line is omitted when historical coverage is insufficient.
